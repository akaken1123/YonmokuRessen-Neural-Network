...
（リプレイバッファ方式）。実際に1世代分だけで29世代回したところTEST3への勝率が
完全に横ばい（毎世代0%）だったため、この蓄積方式に変更した。
...
- 新旧モデルの自動比較・昇格（今は`run_rl_loop.ps1`が毎世代evaluateの結果をログに残すだけで、
  弱くなった場合に自動で前の世代へ切り戻す、といった判断はまだ人間が見て行う）。
