...
評価し、直前のベストより弱ければ**採用しない**（次の世代もベストのままのチェックポイントから
自己対戦・学習をやり直す）。これが無いと、1回の学習で弱くなってしまった場合にそれが
そのまま次の世代の土台になり、弱いポリシーが弱い自己対戦データを生み続けて弱いまま
固定される、という事故が起きる（実際に、蓄積方式にしてもTEST3への勝率が0%のまま
40世代動かなくなり、学習前の`model_base.pt`の方がまだ強い、という状態になっていた）。
