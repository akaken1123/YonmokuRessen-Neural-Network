...
自動で繰り返すPowerShellスクリプト。YonmokuRessenサーバーを別ウィンドウで起動した
状態で、リポジトリのルートから実行する（Windows専用。`.venv`/`.venv-rocm`の両方を使う）。

```powershell
# 今 checkpoints/model_rl_gen2.pt まであるとして、そこから6世代（gen3〜gen8）を寝てる間に回す
.\scripts\run_rl_loop.ps1 -FromGen 2 -Generations 6
```

1世代失敗したらそこで打ち切り、進捗と各世代の評価結果（TEST3への勝率）を
`training_log_日時.txt`に記録する。パラメータ（`-SelfplayGames`、`-SelfplaySimulations`、
`-EvalGames`など）はコマンドラインから上書きできる。

学習（`train_rl`）には、その世代1回分の自己対戦データだけでなく、`-DataGlob`
（既定`data/rl_selfplay_gen*.jsonl`）にマッチする過去の世代のデータも全部合わせて渡す
（リプレイバッファ方式）。学習率・エポック数も`-Lr`（既定`0.001`）・`-TrainEpochs`
（既定`20`）でコマンドラインから上書きできる（`train_rl.py`自体の`--lr`/`--epochs`
にそのまま渡すだけ）。世代を重ねても勝率が頭打ちになる場合、既存の重みに対する
1回の学習ステップが大きすぎて（学習率が高すぎて）安定して積み上がらず、むしろ
ばらつきになっている可能性がある。その場合は弱め（例：`-Lr 0.0001 -TrainEpochs 8`）
を試してみるとよい（ただし下記の評価ノイズの問題を先に確認すること）。

**評価結果のノイズに注意**：`-EvalGames`（既定`10`）が少ないと、勝率の実測値自体に
大きなばらつきが乗る。実際、同じチェックポイントを別のタイミングで評価し直したら
11.1%→0%のように変わった例がある。10局程度では「勝率0%」と「勝率10%」を統計的に
区別できないことが多いので、昇格ゲートの判定がほぼノイズで決まってしまう。学習率や
データ量を変えても勝率が動かないように見える場合、学習が効いていないのではなく
評価が判定できるだけの局数に達していない可能性を疑うこと。`-EvalGames`を30程度に
増やし、評価だけの並列数も`-EvalConcurrency`（既定`4`、`-SelfplayConcurrency`とは別）
で上げて時間を抑えるとよい。

**昇格ゲート**：新しく学習したチェックポイントは、`-Opponent`（既定`DEFAULT`）との対局で
評価し、直前のベストより**厳密に上回らなければ採用しない**（同率も含めて不採用。次の世代も
ベストのままのチェックポイントから自己対戦・学習をやり直す）。これが無いと、1回の学習で
弱くなってしまった場合にそれがそのまま次の世代の土台になり、弱いポリシーが弱い自己対戦データを
生み続けて弱いまま固定される、という事故が起きる（実際に、蓄積方式にしてもTEST3への勝率が
0%のまま40世代動かなくなり、学習前の`model_base.pt`の方がまだ強い、という状態になっていた）。
評価の相手をTEST3ではなく既定でDEFAULTにしているのは、今のモデルの実力だとTEST3では
差が出ず（勝率が常に0%で「良くなったか」を測れない）、DEFAULTの方が今の実力を映す
感度の高い物差しになるため。
